...
- P.164〜165：P.164のAgentCore CLIインストール手順について、CDKのアップデートに伴うエラー回避のため、最新版のAgentCore CLIをインストールしてください。古いプレビュー版のCLIで作成したプロジェクトでは、P.169の `agentcore deploy` 実行時にビルドエラーになる場合があります。本リポジトリの第5章READMEでは、コピペ用のインストールコマンドを修正しています。書籍P.165のmemoに記載のとおり、最新版CLIで作成し直せば、問題なくデプロイできます。
