...
- 【2026/8/14更新】第13章の AgentCore CLI インストール手順について、CDKのアップデートに伴うエラー回避のため、最新版を使う案内に更新しました。
- 【2026/8/11更新】uvコマンドのアップデートにより挙動が変わったため、第1章、第3章、第4章、第5章、第15章のコピペ用コマンドを修正しました。
...
- P.50：uv 0.12以降でも書籍と同じディレクトリ構成で進められるよう、[第1章README](./chapter1/README.md)のコピペ用コマンドに `--no-package` オプションを追加しました。

### 第3章

- P.101：uv 0.12以降でも書籍と同じディレクトリ構成で進められるよう、[第3章README](./chapter3/README.md)のコピペ用コマンドに `--no-package` オプションを追加しました。

### 第4章

- P.133：uv 0.12以降でも書籍と同じディレクトリ構成で進められるよう、[第4章README](./chapter4/README.md)のコピペ用コマンドに `--no-package` オプションを追加しました。
...
- P.170：uv 0.12以降でも書籍と同じディレクトリ構成で進められるよう、[第5章README](./chapter5/README.md)のコピペ用コマンドに `--no-package` オプションを追加しました。

### 第13章

- P.290：AgentCore CLIの依存関係に含まれるAWS CDKがアップデートされたことにより、デプロイ時にエラーが発生するようになりました。書籍P.290のmemoに記載のとおり、AgentCore CLIの最新版を利用すると問題なくデプロイできます。本リポジトリの[第13章README](./chapter13/README.md)に記載しているコピペ用のインストールコマンドも、あわせて修正しています。

### 第15章

- P.361：uv 0.12以降でも書籍と同じディレクトリ構成で進められるよう、[第15章README](./chapter15/README.md)のコピペ用コマンドに `--no-package` オプションを追加しました。

### 付録

- P.435：uv 0.12以降では `uv init` コマンドがパッケージ構成のプロジェクトを作成します。本書のハンズオンでは、各章READMEに記載した `--no-package` オプション付きのコマンドを使用してください。
